...
impl HomophonCypher {
    ...
    pub fn new(text: String) -> Self {
        let sanitized_text:String =text.to_uppercase().chars().filter(|c| c.is_alphanumeric() || c == &'\n').collect();
        let unique_characters: HashSet<char> = sanitized_text.chars().collect();
        let mut available_keys: Vec<char> = KEYS.chars().collect();
        let keyspace:HashMap<char, Vec<char>> = unique_characters.iter().filter(|c| c != &&'\n').map(|c| {
            let vector:Vec<char> = (1..rand::thread_rng().gen_range(3..8))
                .into_iter()
                .map(|_i| (available_keys.remove(rand::thread_rng().gen_range(0..available_keys.len())))).collect();
            (*c,vector)
        }).collect();
